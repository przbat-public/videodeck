...
- **Trzymaj yt-dlp aktualny** — YouTube regularnie psuje starsze wersje. yt-dlp rekomenduje kanał `nightly` (stable bywa „stale and prone to external breakage"); wersję widać w logu startowym serwera (`yt-dlp version: ...`), a gdy pobieranie zaczyna padać błędami „Sign in to confirm you're not a bot"/429, pierwszą próbą jest `yt-dlp -U` albo kanał `nightly`. W `config.json` kanału możesz też włączyć `impersonate: true` (podszywanie pod przeglądarkę, bez cookies).
...
**Wspólny kontrakt API** leży w pakiecie workspace `@videodeck/shared`: schematy zod (`schemas.ts`) to jedyne źródło prawdy o kształtach odpowiedzi, a `api.ts` reeksportuje typy wyprowadzone przez `z.infer` (plus typy żądań i zdarzeń SSE). Serwer, klient i rozszerzenie importują typy jako `@videodeck/shared/api`; klient **parsuje** każdą odpowiedź schematem (parse, don't trust), a testy tras serwera sprawdzają odpowiedzi tymi samymi schematami — typy i walidacja nie mogą się rozjechać. `api.ts` pozostaje types-only (`import type` wymuszany przez ESLint), a `schemas.ts` i `progress.ts` to celowe moduły runtime'owe wspólne dla serwera i rozszerzenia.
...
**Cache na wymiennych dyskach.** Indeks każdego folderu żyje w Elasticsearch pod aliasem wyliczanym ze ścieżki folderu i **przeżywa odpięcie dysku** — po podmianie dysku wyszukiwanie od razu używa aliasów aktualnie podpiętych folderów (poprzedni dysk po prostu nie jest przeszukiwany), a strona statusu pokazuje per folder tylko brakujący indeks (`indeks ES: brak`). Zamiast pełnego reindeksu wystarczy wtedy:
...
| `impersonate`         | `false`   | Dodaje `--impersonate chrome` — podszywanie się pod przeglądarkę bez cookies (bezpieczna alternatywa dla zakazanych `--cookies*`); pomaga, gdy YouTube odpowiada 429 albo traktuje serwer jak bota                                                                                                                                                                                                                                                                                                                                                                                              |
| `concurrentFragments` | `1`       | Liczba równoległych fragmentów pobierania (`-N`, 1-16); wyższe wartości przyspieszają pobieranie, bo YouTube throttluje pojedyncze połączenie                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `sponsorblockRemove`  | `false`   | Dodaje `--sponsorblock-remove sponsor,selfpromo,interaction` — tnie segmenty sponsorskie przy pobieraniu (ffmpeg, cięcie w klatkach kluczowych bez re-enkodu)                                                                                                                                                                                                                                                                                                                                                                                                                                   |
