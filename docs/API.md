...
| `GET /api/folder/list?folderPath=`                      | Contents of `list.json` + download statuses (from the folder index); a collection (`kind: "collection"`) lists its folder index instead, newest upload first |
| `GET /api/folder/summaries[?folderPath=]`               | Counts per folder for the channel console (`videos`, `downloaded`, `notDownloaded`, `stale`, `newestUpdate`); every folder, cached 5 s, or one folder read fresh (the console asks for it when a job of that folder finishes) |
...
**Folder index** (`server/src/services/folderIndex.ts`): every folder holds a hidden `.videos-index.json` file (`videoId → { baseName, videoFile, infoMtime, title }`) plus yt-dlp's `archive.txt`. Both are built from disk on first use (only the header of each `info.json` is read, which is where yt-dlp writes the id and the title) and updated incrementally after every job, so `GET /api/folder/list` no longer parses all `info.json` files. After manually deleting files from the folder, call `POST /api/folder/rebuild-index`. A collection has no jobs to trigger that refresh, so every read of its list or counts first adds the `info.json` files the index has never seen, whatever their mtime (yt-dlp runs in a terminal, files moved in from another folder).
...
| `kind`                | `"channel"` | `"collection"` marks a folder of single downloads (browser extension, yt-dlp in a terminal): no `channelUrl` and no `list.json`. Its video list and counts come from the folder index, and the channel console shows neither the channel warnings nor the playlist and download actions for it. The config form sets it with the "Kolekcja pojedynczych pobrań" box |
