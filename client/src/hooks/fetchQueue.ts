 ...
 *
 * `no-store` on every call: the queue is live state these endpoints poll, and a
 * conditional request can come back as `304 Not Modified`, which `fetch`
 * reports as `ok: false` (the server used to answer exactly that, and the queue
 * controls showed a load error while the queue was healthy).
 */
...
export async function fetchQueueSummary(signal?: AbortSignal): Promise<QueueSummaryResponse> {
  return apiGet('/api/folder/queue/summaries', QueueSummaryResponseSchema, {
    cache: 'no-store',
    ...(signal ? { signal } : {}),
    message: i18n.t('errors.loadQueue'),
  });
...
export async function fetchQueueJobLog(jobId: string, signal?: AbortSignal): Promise<string[] | null> {
  try {
    const body: QueueJobResponse = await apiGet(
      `/api/folder/queue/${encodeURIComponent(jobId)}`,
