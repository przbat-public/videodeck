...
describe('detectPermanentFailure', () => {
  ...
  it('detects private and removed videos', () => {
    expect(detectPermanentFailure(['ERROR: [youtube] a: This video is private'])).toBe('Video jest prywatne');
    expect(detectPermanentFailure(['ERROR: [youtube] a: This video has been removed by the uploader'])).toBe(
      'Video zostało usunięte lub jest niedostępne',
    );
    expect(
      detectPermanentFailure([
